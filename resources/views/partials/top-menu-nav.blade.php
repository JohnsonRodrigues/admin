    ...
    <ul class="nav navbar-nav pull-right">
        ...
        <li class="dropdown dropdown-user">
            ...
            <ul class="dropdown-menu dropdown-menu-default">
                ...
                <li>
                    @if(config('metronic.logout_method') == 'GET' || !config('metronic.logout_method') && version_compare(\Illuminate\Foundation\Application::VERSION, '5.3.0', '<'))
                        <a href="{{ url(config('metronic.logout_url', 'auth/logout')) }}">
                            <i class="icon-key"></i> {{ trans('metronic::metronic.log_out') }}
                        </a>
                    @else
                        <a href="#"
                           onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                            <i class="fa fa-fw fa-power-off"></i> {{ trans('metronic::metronic.log_out') }}
                        </a>
                        <form id="logout-form"
                              action="{{ url(config('metronic.logout_url', 'auth/logout')) }}"
                              method="POST" style="display: none;">
                            @if(config('metronic.logout_method'))
                                {{ method_field(config('metronic.logout_method')) }}
                            @endif
                            {{ csrf_field() }}
                        </form>
                    @endif
